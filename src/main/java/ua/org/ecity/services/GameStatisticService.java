...
public class GameStatisticService {
    ...
    private void saveResultGameForUser(Game game, int winner) {
        User user = userRepository.findById(game.getFirstPlayer());
        if (winner == 1) {
            user.setCounterwin(user.getCounterwin() + 1);
        } else {
            user.setCounterloss(user.getCounterloss() + 1);
        }
